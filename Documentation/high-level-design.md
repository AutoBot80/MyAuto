...
| `services/fill_hero_dms_service` | Playwright DMS (Siebel only: `siebel_dms_playwright.run_hero_siebel_dms_flow`, **BRD §6.1a**; `DMS_SIEBEL_*` / `DMS_REAL_URL_*`); stage-5 **`prepare_vehicle`** order documented in **LLD §2.4d** / **6.72** (left Title drill-in → key/battery → inventory gate → Serial/Features/Pre-check/PDI on dealer stock). DMS fill from **`form_dms.py`** or **`add_sales_staging.payload_json`**; Vaahan helpers stubbed until production automation; reuses open tabs via `handle_browser_opening.get_or_open_site_page`; writes traces under `ocr_output/`. |
