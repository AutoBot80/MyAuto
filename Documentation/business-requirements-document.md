...
| 2a | **Contact / Enquiry — not found** | **Preferred (real Siebel):** when **Find** returns **no contact table row**, navigate to the **Vehicles** view (`DMS_REAL_URL_VEHICLE`), use header **Find → Vehicles**, fill the right **Find** fly-in **VIN** and **Engine#** with chassis/engine from `form_dms_view` (Siebel-style `*` wildcard prefix where the UI expects it), run the query (**Enter**), **read model / year of manufacture / color** from the results grid and/or the **Vehicle Information** detail (e.g. **Dispatch Year** as year), then **Enquiry** tab → **Opportunities List:New** → fill opportunity fields from DB only (e.g. **Finance Required** Y if `customer_master.financier` populated else N, **Booking Order Type** Normal Booking, **Financier**, **UIN Type** Aadhaar Card, **UIN No.** last 4 from `customer_master.aadhar`, **Point of Contact** Customer Walk-in); **Ctrl+S**. **Fallback:** **New enquiry** on contact/enquiry form from `form_dms_view` + **Save** if the vehicle/opportunity path cannot complete. If the booking path applies (step 4b), continue to **Generate Booking** after the record is valid. |
