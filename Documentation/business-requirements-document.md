...
- **Inventory + discount:** **`vehicle_inventory_master`** is keyed by scraped chassis/engine. Per-line **discount** is resolved with **`dealer_ref.subdealer_type`** ( **`to_dealer_id`** ) and **`subdealer_discount_master_ref`** ( **`dealer_id`** = **`from_dealer_id`**, same **`subdealer_type`**, **`valid_flag=Y`**, **model** = **prefix** of DMS model); if none applies, **1500.00**. If the batch header has **`add_transport_cost`** true, **`transport_cost_per_vehicle`** is subtracted from that amount per line (minimum **0**) before **`vehicle_inventory_master.discount`** is written for the order phase and before **`order_line_vehicles`** — **Database DDL** **§12**, **§16**, **2.89** / **2.90** / **2.92**; **LLD** **§2.4e**.
...
- **Commit:** **`challan_master`** holds order/invoice references and totals (including snapshot **`add_transport_cost`** / **`transport_cost_per_vehicle`**); **`challan_details`** links to **`vehicle_inventory_master`** lines. Detail staging rows move to **Committed** after successful commit.
...
| 3.169 | May 2026 | — | **BR-22** / **FR-25** / **§6.9**: Subdealer challan **transport cost** — **`add_transport_cost`**, **`transport_cost_per_vehicle`** on **`challan_master_staging`** / **`challan_master`**; net per-line discount before **`order_line_vehicles`**; UI **New Challan** — **LLD** **§2.4e**, **6.301**; **HLD** **1.172**; **Database DDL** **2.92** |
| 3.170 | May 2026 | — | **FR-18b** / **§6.4**: **CPA Alliance** third-party insurer portal — **`master_ref.comments`** (CPA **https** URL), **`dealer_ref.cpa_insurer`**, **`hero_cpi`** gating (**`cpa_alliance_portal_enabled`**); **`GET /add-sales/create-invoice-eligibility?dealer_id=`**; **`add_alliance_cpa_insurance`**, native **`CPAInsurance`** profile, **`ocr_logs/.../playwright_cpa_*.txt`**, sidecar **`fill_cpa_alliance_insurance`**, **`AddSalesPage`** — **LLD** **6.302**, **HLD** **1.173**, **Database DDL** **2.93** |
