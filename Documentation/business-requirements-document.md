...
| 3 | **Vehicles → Auto Vehicle List** (`DMS_REAL_URL_VEHICLE`) | **Find→Vehicles** with **`*`**-prefixed **VIN (chassis)** and **Engine** partials (same fly-in as Add Enquiry); execute find; open or read the row and determine **stock status** (e.g. **In Transit** vs available for booking/allocation). **Key** partials are used later on the vehicle form (**`prepare_vehicle`** key/battery fill), not as a fallback list search (**LLD** **6.51**). |
