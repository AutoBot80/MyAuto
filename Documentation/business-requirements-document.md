...
| 3.72 | Apr 2026 | — | **Hero Insurance**: **`[DIAG]`** includes **`#root`**-only control list; Sign In tries **`#root`** scope first; if **subfolder** is not sent on **`POST /fill-dms/insurance/hero`**, diagnostics append to dealer **`Playwright_insurance_diag_fallback.txt`** — **LLD** **6.126**, **HLD** **1.62**, **Database DDL** **2.67** |
...
| 3.78 | Apr 2026 | — | **MISP partner login**: **`requestSubmit`** on password form before clicks; post-submit UI snapshot in debug NDJSON when still on partner URL (button label / hints) — **LLD** **6.132**, **HLD** **1.68**, **Database DDL** **2.73** |
| 3.79 | Apr 2026 | — | **FR-18b** / shared browser: **`get_or_open_site_page`** avoids a **second** insurance (or launcher) tab when CDP attaches before navigation; **`www.`** host normalization for tab match — **LLD** **6.133**, **HLD** **1.69**, **Database DDL** **2.74** |
