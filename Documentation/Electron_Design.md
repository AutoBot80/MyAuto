...
| `ENVIRONMENT` | In **`{SAATHI_BASE_DIR}/.env`** (loaded by `electron/sidecar/job_runner.py` before `app.config`). Use **`prod`** or **`production`** (any casing, e.g. **`PROD`**) so `ENVIRONMENT_IS_PRODUCTION` is true in `backend/app/config.py` — enables dealer-side production-only DMS automation such as **auto-click Create Invoice** after order attach (`HERO_DMS_ATTACH_AUTO_CLICK_CREATE_INVOICE`). **Installer:** NSIS writes **`ENVIRONMENT=PROD`** into a **new** `.env`, and on **reinstall/upgrade** if `.env` already exists it **appends** that line only when no line contains **`ENVIRONMENT=`** (`findstr`, `electron/build/installer.nsh` `customInstall`). **Existing dealers:** add the line manually if they never rerun the installer. |
