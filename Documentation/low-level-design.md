...
| POST | `/uploads/scans-v2` | Add Sales v2 upload; server runs `OcrService.process_uploaded_subfolder` in the same request and returns `extraction` with optional `section_timings_ms` (prefetch vs parallel Aadhaar+Details vs merge, insurance, Raw_OCR). **Aadhaar:** UIDAI QR from front and/or back first; **Textract** (and Tesseract on back if needed) fills missing name/DOB/gender/address/state/PIN. **Details sheet:** raster/PDF uses Textract **AnalyzeDocument FORMS**; `.docx` parsed locally. When `OCR_UPLOAD_PARALLEL_TEXTRACT` is true (default), Textract prefetch runs **in parallel with** local QR decode; Aadhaar assembly and Details compile run **in parallel**, then JSON is merged once. |
