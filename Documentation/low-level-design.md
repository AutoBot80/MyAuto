...
| 6.68 | Mar 2026 | — | Video Relation's Name + stage 4 care-of: fill uses **`care_of`** only (DMS Father/Husband line). **`_add_customer_payment`**: activates **Payments** tab before row check; **`_payment_lines_has_existing_row`** requires Payment Lines / transaction markers in the **same** document as the jqgrid row (avoids Contact / enquiry false positives). |
