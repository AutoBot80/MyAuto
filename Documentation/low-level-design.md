...
| POST | `/fill-dms/dms` | DMS only; `DMS_MODE=dummy` runs static HTML under `DMS_BASE_URL`; `DMS_MODE=real` runs `siebel_dms_playwright.run_hero_siebel_dms_flow`. **`skip_find`**: dummy skips contact finder Go, then enquiry form + **Generate booking** + later steps; real Siebel skips Find/mobile search, `goto` **`DMS_REAL_URL_ENQUIRY`** or **`DMS_REAL_URL_CONTACT`**, fills enquiry/customer (mobile on form), **Save** (if found) + **Generate Booking**, then **`DMS_REAL_URL_VEHICLE`**. Normal real flow: `goto` contact, Find → Contact, mobile find, form fill, vehicle search, scrape. Env: `DMS_SIEBEL_*`, `DMS_REAL_URL_*`. |
...
- **Automation contract:** `fill_dms_service.py` requires `customer_id` and `vehicle_id`, loads DMS field values from `form_dms_view`, and drives the dummy DMS in order: **Enquiry** (`"DMS Contact Path"`: `found`, `new_enquiry`, or **`skip_find`** — dummy: skip finder Go, then form + **Generate booking**), S/O or W/o + father/husband, customer budget **89000**, generate booking → **Vehicles** → **PDI** → **Auto Vehicle List** → **Enquiry** (allocate) → **Invoice line** (no Create Invoice) → **Reports**. **`DMS_MODE=real`:** `siebel_dms_playwright.py`; **`skip_find`**: enquiry URL (or contact), customer form + Generate Booking, then vehicle list. Otherwise contact find + vehicle scrape; sets `dms_siebel_forms_filled` on success. **`routers/fill_dms.py`:** *No such vehicle found in DMS* applies when scrape lacks key/frame/engine unless real mode did not complete Siebel automation (`dms_siebel_forms_filled` false).
