...
- **Aadhaar UIDAI QR:** `OcrService._process_aadhar` and `get_extracted_details` decode QR from **`Aadhar.jpg` first**, then merge any missing fields from **`Aadhar_back.jpg`** when present (QR is often on the back). Uses `qr_decode_service.decode_qr_from_image_bytes`, which collects **all** decoded strings across **0°/90°/180°/270°** rotations (deduped) so a non-UIDAI QR in one orientation does not hide the UIDAI payload. `OcrService` picks the **richest** UIDAI-derived customer (by field completeness), not only the first decode. Constructed `address` includes POA parts **city (vtc), district, sub-district, post office** when house/street are absent.
