...
| 6.43 | Mar 2026 | — | ~~Temporary gate **`SIEBEL_DMS_FORCE_FAIL_BEFORE_FIND_CONTACT`**~~ **removed**; replaced by **`SIEBEL_DMS_FORCE_FAIL_BEFORE_FILL_RELATIONS_NAME`** (stop before Relation's Name; logs Find list row counts + enquiry-hint rows) |
...
| 6.65 | Mar 2026 | — | **`_find_contact_mobile_first_grid_counts`** + **`SIEBEL_DMS_FORCE_FAIL_BEFORE_FILL_RELATIONS_NAME`**: video path snapshots list before title sweep; linear path counts live before stage 4 Relation's Name |
