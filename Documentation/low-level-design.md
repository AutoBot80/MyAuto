...
| 6.68 | Mar 2026 | — | Video Relation's Name + stage 4 care-of: fill uses **`care_of`** only (DMS Father/Husband line). **`_add_customer_payment`**: **`_siebel_try_activate_payments_tab`** (multiple tab labels); longer settle wait; **`_payment_lines_has_existing_row(require_amount_like=True)`** before add so template rows do not skip **`+`**; **`+`** only from frames that expose Payment Lines toolbar (no fallback to arbitrary Siebel roots). |
