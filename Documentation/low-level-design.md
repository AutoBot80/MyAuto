...
| 4a | Receipt, **Pre Check**, PDI | If **`in_transit`:** receipt URL → **Process Receipt**; **`_siebel_run_precheck_and_pdi`**: optional **`DMS_REAL_URL_PRECHECK`**, else Pre Check click on **`DMS_REAL_URL_PDI`** before **PDI Submit** (no generic **Submit** — PDI-specific labels only); milestone **Pre check completed** |
...
- **`Playwright_DMS.txt` vehicle visibility:** Header lines **`full_chassis_from_source`**, **`full_engine_from_source`**, **`vehicle_model_from_source`**, **`vehicle_color_from_source`** echo DB-backed DMS values passed into the run. **Add Enquiry** vehicle path scrapes **`full_chassis`** / **`full_engine`** (and model/color) from Siebel **Vehicle Information** after VIN drill-in. **Stage 5** runs **`prepare_vehicle`**, which navigates **Auto Vehicle List**, queries key/chassis/engine, uses **`scrape_siebel_vehicle_row`** (**`frame_num`**, **`engine_num`**, **`model`**, **`color`** on the grid), calls **`_siebel_fill_key_battery_from_dms_values`**, and when **In Transit** runs receipt + **`_siebel_run_precheck_and_pdi`**. Order-line VIN drilldown (cubic capacity, PreCheck/PDI on the line) remains in **`_attach_vehicle_to_bkg`** after **Generate Booking**. Grid keys differ from **`full_chassis`** / **`full_engine`**; notes in the trace state this distinction.
...
| 6.43 | Mar 2026 | — | Temporary gate **`SIEBEL_DMS_FORCE_FAIL_BEFORE_FIND_CONTACT=True`**: hard-stop before any Find-Contact mobile search (video + linear paths), with explicit error/note in `Playwright_DMS.txt`, to allow isolated vehicle-prep hardening |
