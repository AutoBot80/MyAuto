...
| 6.68 | Mar 2026 | — | Video Relation's Name + stage 4 care-of: fill uses **`care_of`** only (DMS Father/Husband line). **`_add_customer_payment`**: **`_siebel_try_activate_payments_tab`**; **`_gather_payment_line_toolbar_roots`** (frame where **Payment Lines List:New** / Save lives); **`_payment_lines_list_has_populated_transaction_number`** in that document — skip **`+`** only when **Transaction #** is populated; post-save verify same rule. Helpers: **`_siebel_root_evaluate`**, **`_siebel_root_has_payment_lines_toolbar`**. |
