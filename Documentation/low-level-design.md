...
- **`_create_order` (video SOP path):** After **Vehicle Sales / My Orders** loads, **`_run_vehicle_sales_my_orders_mobile_search`** runs in the same frame as **Sales Orders List:New (+)** (Find **`s_1_1_1_0`** → **Mobile Phone#** → mobile → Enter → **`ui-jqgrid-btable`**). **invoiced** (meaningful Invoice#) → success with **`ready_for_client_create_invoice`** and **no** atomic master INSERT from Siebel scrape; **pending** / **allocated** → Order# drilldown then **`_attach_vehicle_to_bkg`** (allocated uses **`start_at_order_link_before_apply=True`**); **no_rows** / **unknown_rows** / **error** → full **+** new booking (Contact applet, Ctrl+S, then **`_attach_vehicle_to_bkg`**). The former **Invoice Selected** tab shortcut and legacy post-**+** line-item-only path were removed — **LLD** **6.115**.
