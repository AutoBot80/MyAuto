...
| 5.3 | Mar 2026 | — | **`update_vehicle_master_from_dms`**: maps **full_chassis** / **full_engine**, **sku** → **dms_sku**, **raw_frame_num** / **raw_engine_num**, **dispatch_year** fallback for **year_of_mfg**, **ex_showroom_price** → **`vehicle_ex_showroom_price`**; runs after real Siebel when **`out['vehicle']`** is non-empty (including partial Add Enquiry scrape) |
...
| 6.5 | Mar 2026 | — | Service module renamed to **`fill_hero_dms_service.py`**; Fill DMS now validates `dealer_master.parent_id='100000'` before execution and returns OEM guardrail error for non-Hero dealers |
