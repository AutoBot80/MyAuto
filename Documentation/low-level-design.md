...
| POST | `/fill-dms/dms` | DMS only; `DMS_MODE=dummy` runs static HTML under `DMS_BASE_URL`; `DMS_MODE=real` runs `siebel_dms_playwright.run_hero_siebel_dms_flow`: after `goto` contact URL, sets **Find → Contact** when detectable (native `select` or combobox + menu), then fills mobile (**Mobile Phone #** / **Mobile Number** / **Mobile Phone**, CSS + semantic locators) on `DMS_REAL_URL_CONTACT`, vehicle search on `DMS_REAL_URL_VEHICLE`, scrape first wide grid row, optional `DMS_REAL_URL_*` navigation; writes `DMS_Form_Values.txt`. Env: `DMS_SIEBEL_CONTENT_FRAME_SELECTOR`, `DMS_SIEBEL_MOBILE_ARIA_HINTS`, timeouts. With `real`, `require_login_on_open` is false on auto-open (CDP reuse recommended). Response includes `dms_automation_mode`, `warning` only if real run did not set `dms_siebel_forms_filled`, and **`dms_milestones`**: ordered human labels (e.g. Customer found, Enquiry created, …) for the Add Sales banner. |
