...
| 2a | New customer: basic enquiry + re-find + care | `new_enquiry` path | **No table match** → try **`_add_enquiry_opportunity`** (**Find→Vehicles**, right fly-in **VIN** + **Engine#** with `*` prefix + **Enter**; **`scrape_siebel_vehicle_row`** + detail applet merge must yield **model**, **year_of_mfg** (incl. **Dispatch Year**), **color**; then **Enquiry** tab, **Opportunities List:New**, DB fields, Ctrl+S; scrape merged into **`out['vehicle']`**); on failure → **`_fill_basic_enquiry_details`** + **Save**; **`new_enquiry`** path still uses basic enquiry; mandatory **`_refind_customer_after_enquiry`** when a new enquiry was created; then care-of + **Save**; milestone **Add enquiry saved** / **Enquiry created** |
...
| 4.9 | Mar 2026 | — | After vehicle search: **`_siebel_try_click_vin_search_hit_link`** clicks the **Search Results** / **Siebel Find** VIN drill-down (same pattern as contact mobile hit); detail scrape reads **Model** / **Color** / **Dispatch Year** from table rows and Siebel **input** titles; **`sku`** merged into **`out['vehicle']`** when present |
