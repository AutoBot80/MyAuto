-- Dealer preferred portal insurer label for MISP KYC; exposed on form_insurance_view.
-- When details-sheet insurer fuzzily matches prefer_insurer (>=20%%), automation uses prefer_insurer on KYC.
-- Run after: dealer_ref, 10j_form_insurance_view.sql (view will be recreated here).

ALTER TABLE dealer_ref
    ADD COLUMN IF NOT EXISTS prefer_insurer VARCHAR(255);

COMMENT ON COLUMN dealer_ref.prefer_insurer IS
    'Preferred MISP insurer option label when details insurer is a fuzzy match (at least 20 percent); overrides merged insurer for KYC';

-- Second seed dealer (Arya Agencies); also row id 2 if present in DB
UPDATE dealer_ref
SET prefer_insurer = 'Universal Sompo General Insurance'
WHERE dealer_id IN (2, 100001);
...
COMMENT ON VIEW form_insurance_view IS
    'Single-row projection per sale: customer_master, vehicle_master, latest insurance_master row (by policy_to/year/id), dealer_ref (incl. prefer_insurer); nominee_gender from insurance_master';
