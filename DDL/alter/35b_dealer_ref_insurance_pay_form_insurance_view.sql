-- MISP proposal Payment Mode: dealer_ref.insurance_pay (CC | APD).
-- CC for dealer 100001 (Arya); APD default for all other dealers.
-- Run after: 17a (form_insurance_view with hero_cpi), 34a (cpi_reqd).

ALTER TABLE dealer_ref
    ADD COLUMN IF NOT EXISTS insurance_pay VARCHAR(8);

UPDATE dealer_ref
SET insurance_pay = 'APD'
WHERE insurance_pay IS NULL;

ALTER TABLE dealer_ref
    ALTER COLUMN insurance_pay SET DEFAULT 'APD';

ALTER TABLE dealer_ref
    ALTER COLUMN insurance_pay SET NOT NULL;

ALTER TABLE dealer_ref
    DROP CONSTRAINT IF EXISTS chk_dealer_ref_insurance_pay;

ALTER TABLE dealer_ref
    ADD CONSTRAINT chk_dealer_ref_insurance_pay
    CHECK (insurance_pay IN ('CC', 'APD'));

COMMENT ON COLUMN dealer_ref.insurance_pay IS
    'MISP proposal Payment Mode dropdown: CC (100001 / Arya) or APD (default for other dealers) — see fill_hero_insurance_service';

UPDATE dealer_ref SET insurance_pay = 'CC' WHERE dealer_id = 100001;
UPDATE dealer_ref SET insurance_pay = 'APD' WHERE dealer_id <> 100001;
...
SELECT
    ...
    COALESCE(dr.insurance_pay, 'APD') AS insurance_pay,
...
COMMENT ON VIEW form_insurance_view IS
    'Single-row projection per sale: customer_master, vehicle_master, latest insurance_master row (by policy_to/year/id), dealer_ref (incl. prefer_insurer, hero_cpi, insurance_pay); nominee_gender from insurance_master';
